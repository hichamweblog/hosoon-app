...
export default function ActivityHeatmap() {
  ...
  const { heatmapWeeks, totalMonthsSpan } = useMemo(() => {
    ...
    const loggedDates = Object.keys(dailyLog || {})
      .filter(dateStr => dailyLog[dateStr] && dailyLog[dateStr].tasksCompleted > 0)
      .sort();
    
    const oldestLoggedDate = loggedDates.length > 0 ? new Date(loggedDates[0]) : today;
    const startOfCurrentMonth = new Date(today.getFullYear(), today.getMonth(), 1);
    const startOfOldestMonth = new Date(oldestLoggedDate.getFullYear(), oldestLoggedDate.getMonth(), 1);
    
    const startDate = startOfOldestMonth < startOfCurrentMonth ? startOfOldestMonth : startOfCurrentMonth;
    ...
    const totalDaysToGenerate = Math.max(Math.ceil((diffDays + 1) / 7) * 7, 126);
    ...
    for (let i = totalDaysToGenerate - 1; i >= 0; i--) {
      ...
      else if (tasksCompleted >= 4) colorClass = "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]";
    ...
    const monthsSpan = (today.getFullYear() - startDate.getFullYear()) * 12 + (today.getMonth() - startDate.getMonth()) + 1;
    ...
    <div className="glass-panel rounded-3xl p-6 border border-border/50 overflow-hidden flex flex-col items-center">
      <div className="w-full flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">تتبع نشاطك اليومي في الحفظ والمراجعة</p>
        </div>
      </div>
                ...
                return (
                  <TooltipProvider key={dayData.dateStr} delayDuration={100}>
                    <Tooltip>
                      ...
                      />
                      <TooltipContent className="bg-popover text-popover-foreground border-border text-xs" side="top">
                        <p className="font-bold mb-1" dir="rtl">{dayData.date.toLocaleDateString('ar-DZ', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
      ...
      <div className="w-full flex justify-end items-center gap-2 mt-4 text-xs text-muted-foreground" dir="rtl">
        <span>أقل</span>
